...
odoo.define('pos_retail.promotion', function (require) {
    ...
    models.Order = models.Order.extend({
        ...
        checking_promotion_birthday_match_birthdayof_client: function (promotion) {
            ...
            if (promotion.promotion_birthday && !client) {
                return false
            }
            if (!promotion.promotion_birthday || !client || !client['birthday_date']) {
                return true
            } else {
                var correctly = false;
                ...
                if (promotion['promotion_birthday_type'] == 'day') {
                    if ((birthday_date.date() == today.date()) && (birthday_date.month() == today.month())) {
                        correctly = true
                        console.log('birthday client in this day');
                    }
                }
                if (promotion['promotion_birthday_type'] == 'week') {
                    ...
                    if (startOfWeek <= birthday_date && birthday_date <= endOfWeek) {
                        correctly = true;
                        console.log('birthday client in this week');
                    }
                }
                if (promotion['promotion_birthday_type'] == 'month') {
                    if (birthday_date.month() == today.month()) {
                        correctly = true
                        console.log('birthday client in this month');
                    }
                }
                return correctly
            }
        ...
        checking_prmotion_has_groups: function (promotion) {
             ...
             */
            var client = this.get_client();
            if (!promotion.promotion_group || promotion.promotion_group_ids.length == 0 || !client || !client.group_ids || client.group_ids.length == 0) {
                return true
            } else {
                var correctly = false;
